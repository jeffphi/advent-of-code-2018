...
int GetClosest(vector< vector<Cell> >& grid, UnitKind kind, int r, int c, unordered_map<pair<int,int>, int> list){
    ...
    */
    ...
    if(r>0 && grid[r-1][c].kind == open && grid[r-1][c].pUnit == NULL && !marked){
        vector<pair<int,int> > newList = list;
        newList.push_back(make_pair(r-1,c));
    ...
    if(c>0 &&grid[r][c-1].kind == open && grid[r][c-1].pUnit == NULL && !marked){
        vector<pair<int,int> > newList = list;
        newList.push_back(make_pair(r,c-1));
...
bool Moved(vector< vector<Cell> >& grid, int r, int c, int& newRow, int&  newCol){
    ...
    */
    ...
    unordered_map<pair<int,int>, int> upList;
    upList[make_pair(r,c)] = 0;

    unordered_map<pair<int,int>, int> leftList = upList;
    unordered_map<pair<int,int>, int> rightList = leftList;
    unordered_map<pair<int,int>, int> downList = rightList;
