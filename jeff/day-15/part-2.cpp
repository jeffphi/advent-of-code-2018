...
bool Done(vector< vector<Cell> >& grid, int rounds, int elfPower){
    int numElves = 0;
    int numGoblins = 0;
    int totalHP = 0;
    ...
    if(numElves == 0 || numGoblins == 0){
        ...
        ss << "Done! Full rounds: " << rounds << ", Remaining HP: " << totalHP << ", ElfPwr:" << elfPower << ", Score: " << (rounds*totalHP);
...
bool OptimizeElves(vector<string> input, int elfPower){
    ...
    while(true){
        ...
        numRounds++;
        ClearMoved(grid);
        if(Done(grid, numRounds-1, elfPower)) break;
    }
    ...
    for(int r = 0; r < grid.size(); r++){
        for(int c = 0; c < grid[0].size(); c++){
            if(grid[r][c].pUnit != NULL && grid[r][c].pUnit->kind == elf){
                finalNumElves++;
            }
        }
    }

    return (finalNumElves == numElves);
}
...
int main()
{
    ...
    int elfPower = 4;
    while(true){
        if(OptimizeElves(input, elfPower)) break;
        elfPower++;
    }
